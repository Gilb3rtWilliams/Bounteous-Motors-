...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (userData) => {
    try {
      ...
      return {
        ...
        userData: data.data,
        bio: data.data.bio || "" // Ensure bio is included
      };
    } catch (error) {
  ...
  const updateProfile = async (profileData) => {
    try {
      ...
      if (!data.success) {
        // only update user if backend says success
        return { success: false, message: data.message || "Update failed" };
      }

      // Use returned "user" object correctly
      const updatedUser = {
        ...user,
        ...data.user,   // includes name, email, bio, role
        token: data.token
      };
